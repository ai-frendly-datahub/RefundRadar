...
from refundradar.collector import RateLimiter, collect_sources
...
def test_parallel_collection_reduces_runtime() -> None:
    ...
    with (
        patch("refundradar.collector._collect_single", side_effect=delayed_collect),
        patch("refundradar.collector.get_circuit_breaker_manager", return_value=manager),
...
def test_parallel_collection_isolates_source_errors() -> None:
    ...
    with (
        patch("refundradar.collector._collect_single", side_effect=selective_collect),
        patch("refundradar.collector.get_circuit_breaker_manager", return_value=manager),
        patch.dict(os.environ, {"RADAR_MAX_WORKERS": "5"}, clear=False),
    ):
        articles, errors = collect_sources(sources, category="test", min_interval_per_host=0.0)
...
def test_max_workers_one_preserves_sequential_order() -> None:
    ...
    with (
        patch("refundradar.collector._collect_single", side_effect=ordered_collect),
        patch("refundradar.collector.get_circuit_breaker_manager", return_value=manager),
        patch.dict(os.environ, {"RADAR_MAX_WORKERS": "5"}, clear=False),
    ):
...
def test_env_var_radar_max_workers_is_used() -> None:
    ...
    with (
        patch("refundradar.collector._collect_single", return_value=[]),
        patch("refundradar.collector.get_circuit_breaker_manager", return_value=manager),
        patch("refundradar.collector.ThreadPoolExecutor") as mock_executor,
