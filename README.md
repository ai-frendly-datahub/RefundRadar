...
- **데이터 수집**: 세무청, 보험사 뉴스 및 공고
- **엔티티 분석**: 환급 유형별 키워드 매칭 (세금, 보험, 지원금 등)
- **트렌드 리포트**: DuckDB 저장 + HTML 리포트로 환급/보조금 동향 시각화
...
- 워크플로: `.github/workflows/radar-crawler.yml`
  ...
  - 환경 변수 `RADAR_CATEGORY` 값은 `refund`입니다.
...
- 설정 방법:
  ...
  3) 워크플로 파일의 `RADAR_CATEGORY`가 `refund`인지 확인
