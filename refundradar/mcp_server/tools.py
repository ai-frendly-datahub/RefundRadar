...
def handle_case_finder(*, db_path: Path, query: str = "", days: int = 30, limit: int = 10) -> str:
    """Find refund or dispute cases from recent articles."""
    cutoff = datetime.now(tz=UTC) - timedelta(days=days)
    conn = duckdb.connect(str(db_path), read_only=True)
    try:
        if query.strip():
            rows = conn.execute(
                """
                SELECT title, source, link, entities_json, collected_at
                FROM articles
                WHERE collected_at >= ? AND (
                    LOWER(title) LIKE ? OR LOWER(summary) LIKE ?
                )
                ORDER BY collected_at DESC
                LIMIT ?
                """,
                [cutoff, f"%{query.lower()}%", f"%{query.lower()}%", limit],
            ).fetchall()
        else:
            rows = conn.execute(
                """
    ...
    for title, source, link, entities_json, collected_at in rows:
        entity_tags = ""
        if entities_json:
            try:
                ...
                if entity_names:
                    entity_tags = f" [{', '.join(entity_names)}]"
            except json.JSONDecodeError:
                pass
        lines.append(f"- {title}{entity_tags}")
